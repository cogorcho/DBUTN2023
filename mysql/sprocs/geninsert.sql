...
BEGIN
	...
	getcol: LOOP
		...
		IF cnt = 0 THEN
			IF ispk = 1 THEN
				...
				SET PARCOLS := CONCAT('(p_',colname);
				INSERT INTO TTAB(fila) VALUES (CONCAT(' OUT p_', colname, ' ', datatype));
				SET pkname := colname;
			ELSE
				SET COLS := CONCAT('(p_',colname);
				SET PARCOLS := CONCAT('(',colname);
		...
		ELSE
			IF ispk = 1 THEN
				...
				SET PARCOLS := CONCAT(PARCOLS, ' ,p_' , colname);
				INSERT INTO TTAB(fila) VALUES (CONCAT(',OUT p_', colname, ' ', datatype));
				SET pkname := colname;
			ELSE
				SET COLS := CONCAT(COLS, ' ,' , colname);
	...
	INSERT INTO TTAB(fila) VALUES (CONCAT('    ',  PARCOLS, ');'));
	INSERT INTO TTAB(fila) VALUES (CONCAT('    SET p_', pkname, ' := last_insert_id();'));
